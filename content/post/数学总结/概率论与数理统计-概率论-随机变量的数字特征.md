---
title: "概率论与数理统计-概率论-随机变量的数字特征"
date: 2020-11-27T09:24:48+08:00
draft: false
categories: ["数学"]
series:
- 概率论与数理统计
tags: ["概率论", "随机变量", "随机变量的数字特征", "数学期望", "均值", "方差", "协方差", "相关系数", "矩", "协方差矩阵", "n维正态分布", "二次型", "矩阵合同", "特征函数", "傅里叶变换"]
markup: pandoc
math: true
---
...
参考：https://zh.wikipedia.org/wiki/%E7%89%B9%E5%BE%81%E5%87%BD%E6%95%B0_(%E6%A6%82%E7%8E%87%E8%AE%BA)
参考：https://www.zhihu.com/question/23686709/answer/376439033

随机变量的分布函数和特征函数一体两面。特征函数是随机变量的分布的不同表示形式。

随机变量的特征函数是对其分布函数做傅里叶变换得到的函数，是用傅里叶变换的方法，从频域（概率论中对应的是矩）的角度分析（随机变量的概率）函数。（从这个角度讲，特征函数应该放到概率分布章节，但是这里要用到矩的知识，所以放到了这一章。

### 特征函数的定义

在概率论中, 任何随机变量的特征函数 (缩写：ch.f, 复数形式: ch.f's) 完全定义了它的概率分布。在实直线上, 它由以下公式给出, 其中 $X$ 是任何具有该分布的随机变 量:
$$
\varphi_{X}(t)=\mathrm{E}\left(e^{i t X}\right)
$$
其中t是一个实数, i是虚数单位, E表示期望值。

如果$F_X$是[累积分布函数](https://zh.wikipedia.org/wiki/累积分布函数)，那么特征函数由[黎曼-斯蒂尔切斯积分](https://zh.wikipedia.org/w/index.php?title=黎曼-斯蒂尔切斯积分&action=edit&redlink=1)给出：
$$
...
$$
如果*X*是一个[向量](https://zh.wikipedia.org/wiki/向量空间)值随机变量，我们便取自变量*t*为向量，*tX*为[数量积](https://zh.wikipedia.org/wiki/数量积)。

### 特征函数的意义

随机变量$X$的特征函数定义为：$\varphi_{X}(t)=\mathrm{E}\left(e^{i t X}\right)$

为什么这么定义呢？

#### 从随机变量的数字特征来看

首先，$e^{i t X}$的泰勒展开为：$e^{i t X}=1+\frac{i t X}{1}-\frac{t^{2} X^{2}}{2 !}+\cdots+\frac{(i t)^{n} X^{n}}{n !}$
代入可以推出：
$\begin{aligned} \varphi_{X}(t) &=E\left[e^{i t X}\right] \\ &=E\left(1+\frac{i t X}{1}-\frac{t^{2} X^{2}}{2 !}+\cdots+\frac{(i t)^{n} X^{n}}{n !}\right) \\ &=E(1)+E\left(\frac{i t X}{1}\right)-E\left(\frac{t^{2} X^{2}}{2 !}\right)+\cdots+E\left(\frac{(i t)^{n} X^{n}}{n !}\right)  \end{aligned}$
$\LARGE=1+\frac{i t \overbrace{E[X]}^{\text{一阶矩}}}{1}-\frac{t^{2} \overbrace{E\left[X^{2}\right]}^{\text{二阶矩}}}{2 !}+\cdots+\frac{(i t)^{n} \overbrace{E\left[X^{n}\right]}^{\text{n阶矩}}}{n !}$

说明特征函数包含了分布函数的所有矩，也就是包含了分布函数的所有（数字）特征。

我们对概念做定义时，是通过抽象出其必须的属性，把具有相同属性的内容当作同一个概念。比如，有生命的物体，称为生物；有生命的、可以运动的、可观测的生物，称为动物；哺乳的动物称为哺乳动物。
数学分析中，根据泰勒定理把各阶导数都相同的两个函数认为是同一个函数。
类似的，概率论中，两个随机变量的各种指标/数字特征（期望、方差、偏态、各阶矩）都相同，它们应该是同一种分布。
...
\mathrm{E}\left(e^{i t X}\right)=\int_{-\infty}^{\infty} e^{i t x} f_{X}(x) d x
$$
而对概率密度函数 $f_{X}$ 作傅里叶变换的公式为：
$$
F(t)=\int_{-\infty}^{\infty} f_X(x) e^{-i t x} d x
$$
可见随机变量$X$的**特征函数**$\varphi_{X}(t)$**与** 随机变量$X$的**概率密度函数 $f_{X}$的傅里叶变换**   **共轭**：
$$
\varphi_{X}(t)=\overline{F(t)}
$$
也就是说，特征函数$\varphi_{X}(t)$是密度函数$f_X(x)$的共轭傅里叶变换，这里**共轭影响不大**，**可以看作傅里叶变换**来理解。

即特征函数相当于在频域分析分布函数。

> 注：
>
> **傅里叶级数**：$\tilde{x}(t)=\sum_{k=-\infty}^{+\infty} a_{k} e^{j k w_{0} t}$
> 其中$a_{k}=\frac{1}{T} \int_{-T / 2}^{T / 2} \tilde{x}(t) e^{-j k \omega_{0} t} d t$
>
> 傅里叶级数适用于周期信号中，将周期推广到$\infty$,就变成了傅里叶变换。
>
> **傅里叶变换**（结论）：$x(t)==\frac{1}{2 \pi} \int_{-\infty}^{+\infty} X(j \omega) e^{j \omega t} d \omega(7)$
> 其中，傅里叶级数的包络$X(j \omega)=\int_{-\infty}^{+\infty} x(t) e^{-j \omega t} d t$
>
> 进一步阅读：无穷级数章节。
> 进一步阅读：[傅里叶级数和傅里叶变换是什么关系？](https://www.zhihu.com/question/21665935)
> 进一步阅读：[傅里叶级数和傅里叶变换是什么关系？ - Jason Huang的回答 - 知乎](https://www.zhihu.com/question/21665935/answer/49282739) 
> 进一步阅读：[如何通俗地理解傅立叶变换？](https://www.matongxue.com/madocs/473/)

特征函数的意义的章节内容参考了马同学的博客：https://www.matongxue.com/madocs/742/，以下是他网页的嵌入iframe显示：

<iframe height='400' scrolling='yes' title='马同学的博客：如何理解统计中的特征函数？' src='https://www.matongxue.com/madocs/742/' frameborder='no' allowtransparency='true' allowfullscreen='true' style='width: 100%;'></iframe>
